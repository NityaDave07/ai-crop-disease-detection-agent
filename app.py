...
The crop disease detection model identified the disease as:

**{disease_name.replace('_', ' ')}**

## Immediate Action Plan (Organic)

- Remove severely affected leaves or plant parts where appropriate.
- Keep the affected area clean and improve air circulation.
- Avoid unnecessary overhead watering.
- Monitor nearby plants for similar symptoms.
- Maintain proper field and plant hygiene.

## Immediate Action Plan (Chemical)

Use only agricultural products that are approved and appropriate for the identified crop and disease. Follow the product label and recommended dosage. Consult a local agricultural expert before applying pesticides or fungicides.
...
## Local Agricultural Support (India)

For confirmation and crop-specific treatment recommendations, consult your nearest Krishi Vigyan Kendra (KVK), agricultural university, or local agriculture department.

**Note:** The disease prediction was generated by the trained crop disease detection model. The detailed AI-generated explanation is temporarily unavailable.
"""

    # If no Gemini API key is configured, use fallback
    if not GEMINI_API_KEY:
        print("Gemini API key not configured. Using fallback report.")
    ...
    try:
        ...
        Your Task:
        Based on all the information above, provide a comprehensive and actionable report.
        ...
        print("Gemini returned an empty response. Using fallback report.")
        return fallback_report
